...
listType = {'AllOther' ,'TalinHomozygous'};
...
for i = 1 : length(folder)
    ...
    % Filter to be used is based on user stain choice
    if stainType == 1
    ...
    data1 = regionprops(connectCompTotal, maskimageFA1, 'Area', 'Centroid',...
        'MajorAxisLength', 'MinorAxisLength', 'MeanIntensity', 'Perimeter'); %% Get all the FA data
    
    maskimageFA2 = maskedimageChannelFA2;
    data2 = regionprops(connectCompTotal,maskimageFA2,'MeanIntensity');
...
writetable(rawTable, 'FARawData.xlsx'); %% Write table to excel file
